...
class InferenceLoop:
    ...
    def load_cldm(self) -> None:
        ...
        self.diffusion: Diffusion = instantiate_from_config(OmegaConf.load(self.args.train_cfg).model.diffusion)
    ...
    def load_caption(self, caption_path='/home/user/CE-yubo/Changxin/NTIRE/SUPIR_test1/caption.txt'):
        self.captions_all = {}
        with open(caption_path, 'r') as f:
            captions_lines = f.readlines()
            for captions_line in captions_lines:
                imgname, caption = captions_line.split(':')
                # print(caption)
                self.captions_all[imgname] = caption
        print(f'load image captions from {caption_path}')
    ...
    def run(self) -> None:
        ...
        for lq in self.load_lq():
            ...
            caption = self.captions_all[file_stem]
            print(f'{file_stem}: {caption}')
            caption = (caption)
            '''
            pos_prompt = ", ".join(
